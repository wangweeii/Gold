...
void TestCppClient::reqHistoricalTicks()
{
        ...
        m_pClient->reqHistoricalTicks(19004, ContractSamples::EurGbpFx(), "20180809 15:45:00", "", 100, "BID_ASK", 1, true, TagValueListSPtr());
        //! [reqhistoricalticks]
...
{
        printf("OpenOrder. ID: %ld, %s, %s @ %s: %s, %s, %g, %g, %s, %s\n", orderId, contract.symbol.c_str(), contract.secType.c_str(), contract.exchange.c_str(), order.action.c_str(), order.orderType.c_str(), order.totalQuantity,
                                 ...
                                 const std::string &specialConditions)
{
        printf("Tick-By-Tick. ReqId: %d, TickType: %s, Time: %s, Price: %g, Size: %d, PastLimit: %d, Unreported: %d, Exchange: %s, SpecialConditions:%s\n",
               reqId, (tickType == 1 ? "Last"
                                     : "AllLast"), ctime(&time), price, size, attribs.pastLimit, attribs.unreported, exchange.c_str(), specialConditions.c_str());
}
//! [tickbytickalllast]
