...
void TestCppClient::historicalDataRequests(int number)
{
        ...
        {
                case 2005:
                        m_pClient->reqHistoricalData(2005, ContractSamples::EurUsdFx(), "20170201 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2006:
                        m_pClient->reqHistoricalData(2006, ContractSamples::EurUsdFx(), "20170301 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2007:
                        m_pClient->reqHistoricalData(2007, ContractSamples::EurUsdFx(), "20170401 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2008:
                        m_pClient->reqHistoricalData(2008, ContractSamples::EurUsdFx(), "20170501 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2009:
                        m_pClient->reqHistoricalData(2009, ContractSamples::EurUsdFx(), "20170601 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2010:
                        m_pClient->reqHistoricalData(2010, ContractSamples::EurUsdFx(), "20170701 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2011:
                        m_pClient->reqHistoricalData(2011, ContractSamples::EurUsdFx(), "20170801 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2012:
                        m_pClient->reqHistoricalData(2012, ContractSamples::EurUsdFx(), "20170901 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2013:
                        m_pClient->reqHistoricalData(2013, ContractSamples::EurUsdFx(), "20171001 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2014:
                        m_pClient->reqHistoricalData(2014, ContractSamples::EurUsdFx(), "20171101 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2015:
                        m_pClient->reqHistoricalData(2015, ContractSamples::EurUsdFx(), "20171201 00:00:00", "1 M", "1 min", "MIDPOINT", 1, 1,
                                                     false, TagValueListSPtr());
                        break;
                case 2016:
...
void TestCppClient::historicalDataEnd(int reqId, const std::string &startDateStr, const std::string &endDateStr)
{
        ...
        historicalDataRequests(reqId + 1);
}
//! [historicaldataend]
...
void TestCppClient::historicalDataUpdate(TickerId reqId, const Bar &bar)
{
        ...
        if (source[bar_count]->time != bar.time)
        {
                std::cout << "HistoricalUpdate. ReqId: " << reqId << ", Date: " << source[bar_count]->time << ", Open: " << source[bar_count]->open
                          ...
                          << std::endl;
                fast[bar_count] = fast_alpha * source[bar_count]->close + fast_beta * fast[bar_count - 1];
                slow[bar_count] = slow_alpha * source[bar_count]->close + slow_beta * slow[bar_count - 1];
                std::cout << "The FAST EMA(8) is: " << fast[bar_count] << ", SLOW EMA(55) is: " << slow[bar_count] << std::endl;
                source[++bar_count] = &bar;
...
//! [backTest]
void TestCppClient::backTest(std::string begin)
{
        std::cout << "Begin to BackTest." << bar_count << std::endl;
        total_value   = 9600;
        have_position = 0;
        buy_price     = 0;
        sel_price     = 0;
        ...
        los_count     = 0;
        FILE *fp = fopen("min", "r");
        fscanf(fp, "%s %s %lf %lf %lf %lf", date_time, &date_time[9], &bar_open, &bar_high, &bar_low, &bar_close);
        date_time[8] = '-';
        ...
        while (++i < LINES)
        {
                ...
                date_time[8] = '-';
                current_time = date_time;
                fast[i] = fast_alpha * bar_close + fast_beta * fast[i - 1];
                slow[i] = slow_alpha * bar_close + slow_beta * slow[i - 1];
                // Bar bar;
                ...
                {
                        testEmaCross(i, bar_high, bar_low, bar_close, current_time);
                }
        }
        fclose(fp);
        std::cout << "BackTest completed." << std::endl;
        // std::cout << "Begin to back test." << bar_count << std::endl;
        // for (int i = 240574; i < LINES; i++)
        // {
        //         testEmaCross(i, source[i]->high, source[i]->low, source[i]->close, source[i]->time);
        // }
}
//! [backTest]

//! [testEmaCross]
void TestCppClient::testEmaCross(int i, double high, double low, double close, std::string time)
{
        ...
        if (1 == have_position) //已有多单
        {
                ...
                // 多单盈利超过设定值又回调一半则平仓
                if ((highest - place_price) > stop && close <= (highest - place_price) * 2 / 3 + place_price)
                {
                        sel_price     = (highest - place_price) * 2 / 3 + place_price;
                        // if ((highest - place_price) > stop && close <= place_price)
                        // {
                        //         sel_price = place_price;
                        have_position = 0;
                        ...
                        std::cout << std::fixed << std::setprecision(5) << time << ", STOP LONG: " << sel_price << ", Account: "
                                  << total_value
                ...
                if ((low - place_price) <= -stop)
                {
                        ...
                        // std::cout << bar.time << ", Open: " << bar.open << ", High: " << bar.high << ", Low: " << bar.low << ", Close: " << bar.close
                        //           << std::endl;
                        std::cout << time << ", LOSS LONG: " << sel_price << ", Account: " << total_value << ", ServiceFee: "
                                  << service_fee
                                  << ", WIN: " << win_count << ", LOS: " << los_count << std::endl << std::endl;
                }
        }
        else if (-1 == have_position) //已有空单
        {
                ...
                // 空单盈利超过设定值又回调一半则平仓
                // if ((lowest - place_price) < -stop && close >= (lowest + place_price) / 2)
                if ((lowest - place_price) < -stop && close >= place_price - (place_price - lowest) * 2 / 3)
                {
                        // buy_price     = (lowest + place_price) / 2;
                        buy_price     = place_price - (place_price - lowest) * 2 / 3;
                        // if ((lowest - place_price) < -stop && close >= place_price)
                        // {
                        //         buy_price = place_price;
                        have_position = 0;
                        total_value += (sel_price - buy_price) * quantity;
                        total_value -= commission;
                        service_fee += commission;
                        ...
                        // std::cout << bar.time << ", Open: " << bar.open << ", High: " << bar.high << ", Low: " << bar.low << ", Close: " << bar.close
                        //           << std::endl;
                        std::cout << time << ", STOPSHORT: " << buy_price << ", Account: " << total_value << ", ServiceFee: "
                                  << service_fee
                                  << ", WIN: " << win_count << ", LOS: " << los_count << std::endl << std::endl;
                }
                // 空单未盈利直接止损
                if ((high - place_price) >= stop)
                {
                        ...
                        // std::cout << bar.time << ", Open: " << bar.open << ", High: " << bar.high << ", Low: " << bar.low << ", Close: " << bar.close
                        //           << std::endl;
                        std::cout << time << ", LOSSSHORT: " << buy_price << ", Account: " << total_value << ", ServiceFee: "
                                  << service_fee << ", WIN: " << win_count << ", LOS: " << los_count << std::endl << std::endl;
                }
        }
        if (fast[i - 1] < slow[i - 1] && fast[i] >= slow[i]) // 快线上穿慢线
        {
                if (0 == have_position)
                {
                        ...
                        std::cout << time << ", OPON LONG: " << buy_price << ", Account: " << total_value << ", Quantity: " << quantity
                                  << ", ServiceFee: " << service_fee << std::endl;
                }
                else if (-1 == have_position)
                {
                        ...
                        service_fee += commission;
                        sel_price > buy_price ? win_count++ : los_count++;
                        std::cout << time << ", CLOS SHOT: " << buy_price << ", Account: " << total_value << ", ServiceFee: "
                                  << service_fee << ", WIN: " << win_count << ", LOS: " << los_count << std::endl << std::endl;
                        buy_price     = close;
                        place_price   = fast[i];
                        highest       = close;
                        ...
                        quantity      = (int(total_value) * beishu / 10000) * 10000;
                        commission    = quantity / 10000;
                        std::cout << time << ", OPEN LONG: " << buy_price << ", Account: " << total_value << ", Quantity: " << quantity
                                  << ", ServiceFee: " << service_fee << std::endl;
                        // std::cout << bar.time << ", Open: " << bar.open << ", High: " << bar.high << ", Low: " << bar.low << ", Close: " << bar.close
                        //           << std::endl;
                }
        ...
        if (fast[i - 1] >= slow[i - 1] && fast[i] < slow[i]) // 快线下穿慢线
        {
                if (0 == have_position)
                {
                        ...
                        quantity      = (int(total_value) * beishu / 10000) * 10000;
                        commission    = quantity / 10000;
                        std::cout << time << ", OPEN SHOT: " << sel_price << ", Account: " << total_value << ", Quantity: " << quantity
                                  << ", ServiceFee: " << service_fee << std::endl;
                }
                else if (1 == have_position)
                {
                        ...
                        service_fee += commission;
                        sel_price > buy_price ? win_count++ : los_count++;
                        std::cout << time << ", CLOS LONG: " << sel_price << ", Account: " << total_value << ", ServiceFee: "
                                  << service_fee << ", WIN: " << win_count << ", LOS: " << los_count << std::endl << std::endl;
                        sel_price     = close;
                        place_price   = fast[i];
                        lowest        = close;
...
void TestCppClient::testMACD(const Bar &bar)
{
        ...
        if (1 == have_position)
        {
                ...
                if ((bar.low - place_price) <= -stop)
                {
                        ...
                        service_fee += commission;
                        sel_price > buy_price ? win_count++ : los_count++;
                        std::cout << bar.time << ", STPLOSLNG: " << sel_price << ", Account: " << total_value << ", Service fee: " << service_fee
                                  << ", Win: " << win_count << ", Los: " << los_count << std::endl << std::endl;
                }
        }
        else if (-1 == have_position)
        {
                ...
                if ((lowest - place_price) < -stop && bar.close >= (lowest + place_price) / 2)
                {
                        ...
                        service_fee += commission;
                        sel_price > buy_price ? win_count++ : los_count++;
                        std::cout << bar.time << ", STOP SHOT: " << buy_price << ", Account: " << total_value << ", Service fee: " << service_fee
                                  << ", Win: " << win_count << ", Los: " << los_count << std::endl << std::endl;
                }
                // 空单未盈利直接止损
                if ((bar.high - place_price) >= stop)
                {
                        ...
                        service_fee += commission;
                        sel_price > buy_price ? win_count++ : los_count++;
                        std::cout << bar.time << ", STPLOSHOT: " << buy_price << ", Account: " << total_value << ", Service fee: " << service_fee
                                  << ", Win: " << win_count << ", Los: " << los_count << std::endl << std::endl;
                }
        }
...
TestCppClient::TestCppClient() : m_osSignal(2000)//2-seconds timeout
        , m_pClient(new EClientSocket(this, &m_osSignal)), m_state(ST_CONNECT), m_sleepDeadline(0), m_orderId(0), m_pReader(0), m_extraAuth(false)
{
}
//! [socket_init]
...
void TestCppClient::marketDepthOperations()
{
        ...
        //! [reqmarketdepth]
        m_pClient->reqMktDepth(2001, ContractSamples::EurUsdFx(), 5, TagValueListSPtr());
        //! [reqmarketdepth]
        std::this_thread::sleep_for(std::chrono::seconds(2));
...
void TestCppClient::realTimeBars()
{
        ...
        //! [reqrealtimebars]
        m_pClient->reqRealTimeBars(3001, ContractSamples::EurUsdFx(), 5, "MIDPOINT", true, TagValueListSPtr());
        //! [reqrealtimebars]
        std::this_thread::sleep_for(std::chrono::seconds(2));
...
{
        m_pClient->reqContractDetails(209, ContractSamples::EurUsdFx());
        std::this_thread::sleep_for(std::chrono::seconds(2));
        //! [reqcontractdetails]
...
void TestCppClient::hedgeSample()
{
        ...
        m_pClient->placeOrder(m_orderId++, ContractSamples::EurUsdFx(), hedge);
        //! [hedgesubmit]
...
void TestCppClient::testAlgoSamples()
{
        ...
        AvailableAlgoParams::FillAccumulateDistributeParams(baseOrder, 10, 60, true, true, 1, true, true, "20161010-12:00:00 GMT",
        ...
        //! [jeff_vwap_algo]
        AvailableAlgoParams::FillJefferiesVWAPParams(baseOrder, "10:00:00 EST", "16:00:00 EST", 10, 10, "Exclude_Both", 130, 135, 1, 10, "Patience",
                                                     false, "Midpoint");
        m_pClient->placeOrder(m_orderId++, ContractSamples::JefferiesContract(), baseOrder);
        //! [jeff_vwap_algo]

        //! [csfb_inline_algo]
        AvailableAlgoParams::FillCSFBInlineParams(baseOrder, "10:00:00 EST", "16:00:00 EST", "Patient", 10, 20, 100, "Default", false, 40, 100, 100,
                                                  35);
...
{
        //! [reqHeadTimeStamp]
        m_pClient->reqHeadTimestamp(14001, ContractSamples::EurUsdFx(), "MIDPOINT", 1, 1);
        //! [reqHeadTimeStamp]
        std::this_thread::sleep_for(std::chrono::seconds(1));
        //! [cancelHeadTimestamp]
...
{
        //! [reqhistoricalticks]
        m_pClient->reqHistoricalTicks(19001, ContractSamples::IBMUSStockAtSmart(), "20170621 09:38:33", "", 10, "BID_ASK", 1, true,
                                      TagValueListSPtr());
        m_pClient->reqHistoricalTicks(19002, ContractSamples::IBMUSStockAtSmart(), "20170621 09:38:33", "", 10, "MIDPOINT", 1, true,
                                      TagValueListSPtr());
        m_pClient->reqHistoricalTicks(19003, ContractSamples::IBMUSStockAtSmart(), "20170621 09:38:33", "", 10, "TRADES", 1, true,
                                      TagValueListSPtr());
        //! [reqhistoricalticks]
...
void TestCppClient::reqTickByTickData()
{
        ...
        m_pClient->reqTickByTickData(20004, ContractSamples::EurUsdFx(), "MidPoint", 0, false);
        ...
        m_pClient->reqTickByTickData(20008, ContractSamples::EurUsdFx(), "MidPoint", 10, true);
        //! [reqtickbytick]
...
void TestCppClient::tickEFP(TickerId tickerId, TickType tickType, double basisPoints, const std::string &formattedBasisPoints, double totalDividends,
                            int holdDays, const std::string &futureLastTradeDate, double dividendImpact, double dividendsToLastTradeDate)
{
        printf("TickEFP. %ld, Type: %d, BasisPoints: %g, FormattedBasisPoints: %s, Total Dividends: %g, HoldDays: %d, Future Last Trade Date: %s, Dividend Impact: %g, Dividends To Last Trade Date: %g\n",
               tickerId, (int) tickType, basisPoints, formattedBasisPoints.c_str(), totalDividends, holdDays, futureLastTradeDate.c_str(),
...
{
        printf("OpenOrder. ID: %ld, %s, %s @ %s: %s, %s, %g, %g, %s, %s\n", orderId, contract.symbol.c_str(), contract.secType.c_str(),
               contract.exchange.c_str(), order.action.c_str(), order.orderType.c_str(), order.totalQuantity,
               order.cashQty == UNSET_DOUBLE ? 0 : order.cashQty, ostate.status.c_str(), order.dontUseAutoPriceForHedge ? "true" : "false");
        if (order.whatIf)
        {
                printf("What-If. ID: %ld, InitMarginBefore: %s, MaintMarginBefore: %s, EquityWithLoanBefore: %s, InitMarginChange: %s, MaintMarginChange: %s, EquityWithLoanChange: %s, InitMarginAfter: %s, MaintMarginAfter: %s, EquityWithLoanAfter: %s\n",
                       orderId, Utils::formatDoubleString(ostate.initMarginBefore).c_str(),
                       Utils::formatDoubleString(ostate.maintMarginBefore).c_str(), Utils::formatDoubleString(ostate.equityWithLoanBefore).c_str(),
                       Utils::formatDoubleString(ostate.initMarginChange).c_str(), Utils::formatDoubleString(ostate.maintMarginChange).c_str(),
                       Utils::formatDoubleString(ostate.equityWithLoanChange).c_str(), Utils::formatDoubleString(ostate.initMarginAfter).c_str(),
                       Utils::formatDoubleString(ostate.maintMarginAfter).c_str(), Utils::formatDoubleString(ostate.equityWithLoanAfter).c_str());
        }
}
//! [openorder]
                                    ...
                                    double unrealizedPNL, double realizedPNL, const std::string &accountName)
{
        printf("UpdatePortfolio. %s, %s @ %s: Position: %g, MarketPrice: %g, MarketValue: %g, AverageCost: %g, UnrealizedPNL: %g, RealizedPNL: %g, AccountName: %s\n",
               (contract.symbol).c_str(), (contract.secType).c_str(), (contract.primaryExchange).c_str(), position, marketPrice, marketValue,
...
void TestCppClient::execDetails(int reqId, const Contract &contract, const Execution &execution)
{
        printf("ExecDetails. ReqId: %d - %s, %s, %s - %s, %ld, %g, %d\n", reqId, contract.symbol.c_str(), contract.secType.c_str(),
               contract.currency.c_str(), execution.execId.c_str(), execution.orderId, execution.shares, execution.lastLiquidity);
}
//! [execdetails]
...
//! [updatemktdepthl2]
void TestCppClient::updateMktDepthL2(TickerId id, int position, const std::string &marketMaker, int operation, int side, double price, int size)
{
...
//! [scannerdata]
void
TestCppClient::scannerData(int reqId, int rank, const ContractDetails &contractDetails, const std::string &distance, const std::string &benchmark,
                           const std::string &projection, const std::string &legsStr)
{
        printf("ScannerData. %d - Rank: %d, Symbol: %s, SecType: %s, Currency: %s, Distance: %s, Benchmark: %s, Projection: %s, Legs String: %s\n",
               ...
               contractDetails.contract.currency.c_str(), distance.c_str(), benchmark.c_str(), projection.c_str(), legsStr.c_str());
}
//! [scannerdata]
...
{
        printf("Position. %s - Symbol: %s, SecType: %s, Currency: %s, Position: %g, Avg Cost: %g\n", account.c_str(), contract.symbol.c_str(),
               contract.secType.c_str(), contract.currency.c_str(), position, avgCost);
}
//! [position]
...
//! [positionmulti]
void TestCppClient::positionMulti(int reqId, const std::string &account, const std::string &modelCode, const Contract &contract, double pos,
                                  double avgCost)
{
        printf("Position Multi. Request: %d, Account: %s, ModelCode: %s, Symbol: %s, SecType: %s, Currency: %s, Position: %g, Avg Cost: %g\n", reqId,
               account.c_str(), modelCode.c_str(), contract.symbol.c_str(), contract.secType.c_str(), contract.currency.c_str(), pos, avgCost);
}
//! [positionmulti]
...
//! [accountupdatemulti]
void TestCppClient::accountUpdateMulti(int reqId, const std::string &account, const std::string &modelCode, const std::string &key,
                                       const std::string &value, const std::string &currency)
{
        printf("AccountUpdate Multi. Request: %d, Account: %s, ModelCode: %s, Key, %s, Value: %s, Currency: %s\n", reqId, account.c_str(),
               modelCode.c_str(), key.c_str(), value.c_str(), currency.c_str());
}
//! [accountupdatemulti]
...
void TestCppClient::mktDepthExchanges(const std::vector<DepthMktDataDescription> &depthMktDataDescriptions)
{
        ...
        for (unsigned int i = 0; i < depthMktDataDescriptions.size(); i++)
        {
                printf("Depth Mkt Data Description [%d] - exchange: %s secType: %s listingExch: %s serviceDataType: %s aggGroup: %s\n", i,
                       depthMktDataDescriptions[i].exchange.c_str(), depthMktDataDescriptions[i].secType.c_str(),
...
//! [tickNews]
void
TestCppClient::tickNews(int tickerId, time_t timeStamp, const std::string &providerCode, const std::string &articleId, const std::string &headline,
                        const std::string &extraData)
{
        printf("News Tick. TickerId: %d, TimeStamp: %s, ProviderCode: %s, ArticleId: %s, Headline: %s, ExtraData: %s\n", tickerId,
               ctime(&(timeStamp /= 1000)), providerCode.c_str(), articleId.c_str(), headline.c_str(), extraData.c_str());
}
//! [tickNews]
...
//! [historicalNews]
void TestCppClient::historicalNews(int requestId, const std::string &time, const std::string &providerCode, const std::string &articleId,
                                   const std::string &headline)
{
        printf("Historical News. RequestId: %d, Time: %s, ProviderCode: %s, ArticleId: %s, Headline: %s\n", requestId, time.c_str(),
               providerCode.c_str(), articleId.c_str(), headline.c_str());
}
//! [historicalNews]
...
void TestCppClient::historicalTicksBidAsk(int reqId, const std::vector<HistoricalTickBidAsk> &ticks, bool done)
{
        ...
        {
                std::time_t t = tick.time;
                std::cout << "Historical tick bid/ask. ReqId: " << reqId << ", time: " << ctime(&t) << ", mask: " << tick.mask << ", price bid: "
                          << tick.priceBid << ", price ask: " << tick.priceAsk << ", size bid: " << tick.sizeBid << ", size ask: " << tick.sizeAsk
                          << std::endl;
        }
}
//! [historicalticksbidask]
...
void TestCppClient::historicalTicksLast(int reqId, const std::vector<HistoricalTickLast> &ticks, bool done)
{
        ...
        {
                std::time_t t = tick.time;
                std::cout << "Historical tick last. ReqId: " << reqId << ", time: " << ctime(&t) << ", mask: " << tick.mask << ", price: "
                          << tick.price << ", size: " << tick.size << ", exchange: " << tick.exchange << ", special conditions: "
                          << tick.specialConditions << std::endl;
        }
}
//! [historicaltickslast]
...
//! [tickbytickalllast]
void
TestCppClient::tickByTickAllLast(int reqId, int tickType, time_t time, double price, int size, const TickAttrib &attribs, const std::string &exchange,
                                 const std::string &specialConditions)
{
        printf("Tick-By-Tick. ReqId: %d, TickType: %s, Time: %s, Price: %g, Size: %d, PastLimit: %d, Unreported: %d, Exchange: %s, SpecialConditions:%s\n",
               reqId, (tickType == 1 ? "Last" : "AllLast"), ctime(&time), price, size, attribs.pastLimit, attribs.unreported, exchange.c_str(),
...
void TestCppClient::setStep(int step_fast, int step_slow)
{
        ...
        fast_alpha = 2.0 / (fast_step + 1);
        slow_alpha = 2.0 / (slow_step + 1);
        fast_beta  = 1 - fast_alpha;
        slow_beta  = 1 - slow_alpha;
}
